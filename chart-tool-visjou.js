(function (global, factory) {
	...
	var debounce = 500;
	var tipTimeout = 5000;
	var ratioMobile = 1.25;
	var ratioDesktop = 0.75;
	...
	var chartSettings = {
	  ...
	  dimensions: {
	    ...
	    height: function() {
	      var ratioScale = linear$1().range([0, 1700]).domain([this.width * this.ratioMobile, this.width * this.ratioDesktop]);
	...
	function base(container, obj) {
	  ...
	  chartBase
	    ...
	    .attrs({
	      ...
	      'x': 0,
	      'y': 0,
	      'width': obj.dimensions.computedWidth() - margin.top - margin.top,
	      'height': obj.dimensions.computedHeight() - margin.top - margin.top,
	...
	function footer(container, obj) {
	  ...
	  if (obj.source !== '' || obj.editable) {
	    ...
	    var copyright = footerGroup.append('div')
	      .attr('class', ((obj.prefix) + "chart_copyright-text"))
	      .text('© DW');
...
// hier wird die Y-Achsenbeschriftung verschoben
	...
	function center(scale) {
	  var offset = Math.max(0, scale.bandwidth() - 1) ; // Adjust for 0.5px offset.
	...
	function axis(orient, scale) {
	  ...
	  function axis(context) {
	    var values = tickValues == null ? (scale.ticks ? scale.ticks.apply(scale, tickArguments) : scale.domain()) : tickValues,
	        ...
	        path2 = selection.selectAll(".domain2").data([null]),
	    ...
	    //hier wird die Achse verschoben
	     ...
	     path2 = path2.merge(path2.enter().insert("path2", ".tick")
	        .attr("class", "domain2")
	        .attr("stroke", "#F1f3f5")
	    	.attr("transform", "translate(0,0)"));
...
//  hier werden alle x und y Achsenberschriftung (Ticks) nach position(d) verschoben.
	...
	function appendXAxis(axisGroup, obj, scale, axis, axisType) {
	  ...
	  axisGroup
	    .attr('transform', ("translate(00, " + (obj.dimensions.yAxisHeight()) + ")"));
	...
	function linearAxis(obj, axis, axisNode, axisSettings) {
	  ...
	  if (axisSettings.axisType === 'yAxis') {
	    axisNode.selectAll('.tick text')
	      // .attr('transform', 'translate(0,0)')
	      //  hier kann die y- Achsenbeschriftung verschoben werden
	...
	function repositionTextY(text, dimensions, textX) {
	  text.attrs({
	    'transform': ("translate(" + (dimensions.computedWidth() / 6) + ",0)"),
	    'x': -20	
	  });
	}
	...
	function lineChart(node, obj) {
	  ...
	  if (xScaleObj.obj.type === 'ordinal') {
	    xScale
	      .range([0, (obj.dimensions.tickWidth() /2)])
	      .padding(10);
	  }
 ...
 // hier wird die gezeichnete Grafik verschoben
	  ...
	  seriesGroup.append('path')
	    ...
	    .attrs({
	     ...
	     seriesGroup.append('path2')
	    .datum(obj.data.data)
	    .attrs({
	      'transform': ("translate(" + (obj.dimensions.computedWidth() - obj.dimensions.tickWidth()) + 50 + ",0)"),
	      'class': function () {
	        return ((obj.prefix) + "line " + (obj.prefix) + "line-" + (obj.seriesHighlight()) + " " + (obj.prefix) + "highlight");
	      },
	      'd': hLine
	...
	function plot(node, obj) {
	  switch(obj.options.type) {
	    ...
	    case 'area':
	      return obj.options.stacked ? stackedAreaChart(node, obj) : areaChart$1(node, obj);
	    case 'bar':
	...
	function hideTips(tipNodes, obj) {
	  ...
	  if (obj.options.type === 'donut' || obj.options.type === 'column' || obj.options.type === 'bar' ) {
	...
	function appendTipGroup(node, obj) {
	  ...
	  var tipNode = svg.append('g')
	    .attrs({
	      ...
	      'transform': ("translate(0," + (obj.dimensions.margin.top) + ")"),
...
// die einzig relevante Tip Funktion für unser Steigungsdiagramm
	...
	function donutChartTips(tipNodes, obj) {
	    ...
	    for (var prop in dispatchFunctions) {
	      if (dispatchFunctions.hasOwnProperty(prop)) {
	        if (Object.keys(dispatcher._).indexOf(prop) > -1) {
	          ...
	          console.log(("Chart SOUP bietet keinen " + prop + " dispatcher."));
	        }
	      }
	    }
