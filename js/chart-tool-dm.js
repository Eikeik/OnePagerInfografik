...
(function (global, factory) {
	...
	var _ctx = function (fn, that, length) {
	  ...
	  switch (length) {
	    ...
	    };        c.href = "chart-tool.css?token=0"; c.rel = "stylesheet";
        j.src = "chart-tool.js?token=0"; j.async = true; j.defer = true;
	...
	var chartSettings = {
	  ...
	  dimensions: {
	    ...
	    computedWidth: function() {
	      return this.width - this.margin.left - this.margin.right - 100;
	    },
	    height: function() {
	      var ratioScale = linear$1().range([300, 600]).domain([this.width * this.ratioMobile, this.width * this.ratioDesktop]);
	    ...
	    tickWidth: function() {
	    	// hier wird die maximale Balkenlänge verändert
	      // return (this.computedWidth() - (this.labelWidth + this.yAxisPaddingRight) - 20);
	      return (this.computedWidth() - (this.computedWidth() / 5) );
	    },
	...
	function base(container, obj) {
	  ...
	  var chartBase = select(container)
	    ...
	    .attrs({
	      ...
	      // hier wird die Breite des SVGs bestimmt
	      'width': obj.dimensions.tickWidth() + obj.dimensions.labelWidth + 25,
	  ...
	  chartBase
	    ...
	    .attrs({
	      ...
	      'width': obj.dimensions.tickWidth() + obj.dimensions.labelWidth + 25,
	      'height': obj.dimensions.tickWidth(),
...
// verschiebt Achsenbeschriftung
	    axisNode.selectAll('text').attr('y', dimensions.barHeight - 3);
	...
	function axisCleanup(node, obj, xAxisObj, yAxisObj) {
	  ...
	  newXAxisObj.node
	    .attr('transform', ("translate(0, " + (obj.dimensions.computedHeight() - obj.dimensions.xAxisHeight) + ")"));
	...
	function barChart(node, obj) {
	  ...
	  if (!obj.exportable || !obj.exportable.height) {
	    ...
	    // fixed height, so transform accordingly and modify the dimension function and parent rects , plus zwanzig!! 
	    select(node.node().parentNode)
	      .attr('height', function () {
	        var margin = obj.dimensions.margin;
	        return obj.dimensions.computedHeight() + 0 + margin.top + margin.bottom ;
	      });

	    select(node.node().parentNode).select(("." + (obj.prefix) + "bg"))
	      .attr('height', obj.dimensions.computedHeight()+ 0 + margin.top + margin.bottom );
