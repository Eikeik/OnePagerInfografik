...
(function (global, factory) {
	...
	function footer(container, obj) {
	  ...
	  if (obj.source !== '' || obj.editable) {
	    ...
	    var copyright = footerGroup.append('div')
	      ...
	      // copyright.append('image')
	      //   .attr('class', `${obj.prefix}copyright-logo`)
	      //    .attr("xlink:href", "fliege.png")
	      //     .attr("x", 8)
	      // .attr("y", 8)
	      // .attr("width", 20)
	      // .attr("height", 20);
	...
	function discreteAxis(axisNode, scale, axis, axisSettings, dimensions) {
	  ...
	  if (axisSettings.axisType === 'xAxis') {
	    ...
	    //hier weiter machen oder die andere Version
	    axisNode.selectAll('text').attr('x', yAxis.paddingRight);
	    axisNode.selectAll('text').attr('y', dimensions.barHeight);
	...
	function pieChart(node, obj) {
	  ...
	  var yAxisObj = new Axis(node, obj, yScale, 'yAxis');
	  ...
	  var seriesGroup = node.append('g')
	    ...
	    .attr('transform', "translate(0,0)");
	  var singlePie = yScale.bandwidth() / obj.data.seriesAmount;
	  ...
	  var loop = function ( i ) {
	    ...
	    var pieItem = seriesItem
	      .selectAll(("." + (obj.prefix) + "pie"))
	      .data(obj.data.data).enter()
	      .append('g')
	      .attrs({
	        'class': ((obj.prefix) + "pie " + (obj.prefix) + "pie-" + i),
	        'data-series': i,
	   ...
	   pieItem.append('rect')
	    ...
	    pieItem.append('text')
	      .attrs({
	        'x': 0,
	        'y': (i * singlePie),
	        'class': ((obj.prefix) + "pie-label")
	      })
	      .text(function (d, j) {
	        if (d.series[i].val && d.series[i].val !== '__undefined__') {
	          var val = setLabelFormat(obj.xAxis.format, d.series[i].val);
	  ...
	  var loop$1 = function ( i ) {
	    series[i].selectAll(("." + (obj.prefix) + "pie"))
	      .attrs({
	        'width': function (d) {
	          if (d.series[i].val && d.series[i].val !== '__undefined__') {
	            return Math.abs(xScale(d.series[i].val) - xScale(0));
	...
	function appendTipGroup(node, obj) {
	  ...
	  var tipBox = tipNode.append('g')
	    .attrs({
	      ...
	      'transform': ("translate(" + (obj.dimensions.computedWidth() - obj.dimensions.tickWidth()) + "," + (obj.dimensions.computedWidth()) + ")")
	    });
